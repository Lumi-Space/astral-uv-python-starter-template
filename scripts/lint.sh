...
echo "Starting code linting with ruff..."
...
# Run the linter
if ! uvx ruff check --output-format=github "$DIRECTORY" --config "$CONFIG_FILE"; then
    error_exit "Linting failed. Please check the errors above and try again."
fi

# **************************************************************************************

echo "Linting complete. Logs are available in '$LOG_FILE'."
